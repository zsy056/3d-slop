//
// Yet Another Dbricks Brick Generator
// by Badger
...
// and on
// Adjustable Improved Dbricks Brick Generator with Text
...
// Dbricks have unsharp corners for safety, printing thinner layers improves roundness
...
module standard_brick(nx, ny, heightfact, stud_type=3, sparse_bottom=1) {
...
module stud_extruded(x=base_unit,y,z,r) {
...
module ridgeset(x, y, angle, n, height){
	translate([x, y, 0]) rotate([0, 0, angle]) {
		for (i=[1:2*n-1]){
			// Extended Ridge 
			/*FIXME most likely not ported correctly from rounded_dbricks_generator_with_text.scad*/
			translate([(i*base_unit - ridge_thickness)/2,0,0]) rotate([90,0,90]) {
